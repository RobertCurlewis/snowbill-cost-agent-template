...
tables (
    SNOWFLAKE.ACCOUNT_USAGE.AUTOMATIC_CLUSTERING_HISTORY primary key (START_TIME) comment='The table contains records of automatic clustering activity performed on database tables. Each record captures a discrete clustering operation, including the time period of the activity, resource consumption in credits, the volume of data and rows reclustered, and the associated table, schema, and database identifiers.',
    SNOWFLAKE.ACCOUNT_USAGE.PIPE_USAGE_HISTORY primary key (START_TIME) comment='The table contains records of historical usage activity for data ingestion pipes. Each record captures a time-bounded interval of pipe activity, including credit consumption, billing, and the volume of data and files processed.',
    SNOWFLAKE.ACCOUNT_USAGE.REPLICATION_USAGE_HISTORY comment='The table contains records of database replication activity over time. Each record captures a specific replication event, including the database involved, the duration of the replication, and associated resource consumption in terms of credits used and data transferred.',
    SNOWFLAKE.ACCOUNT_USAGE.SEARCH_OPTIMIZATION_HISTORY comment='The table contains records of search optimization service usage within a Snowflake account. Each record captures a time-bounded period of activity, identifying the specific database objects involved and the computational credits consumed during that interval.'
)
facts (
